  ...
  try {
    ...
    if (format === 'csv') {
      ...
      return { success: true, message: "CSV data prepared.", data: csvData };
        ...
        wsData.push([{v: `Marks for ${submissionData.assessmentName}`, s:{font:{bold:true, sz:14, color: {rgb: "FF2C3E50"}}, alignment: { horizontal: "center" }}}]);
        wsData.push([{v: `Generated on: ${new Date().toLocaleDateString()}`, s:{font:{italic:true, sz:10, color: {rgb: "FF7F8C8D"}}, alignment: { horizontal: "center"}}}]);
        wsData.push([]);

        wsData.push(tableHeaders.map(h => ({ v: h, s: { font: { bold: true, color: {rgb: "FFFFFFFF"}}, fill: { fgColor: { rgb: "FF34495E" } }, alignment: {horizontal: "center"} } })));

        studentMarksData.forEach(row => {
            wsData.push(tableHeaders.map(header => {
        ...
        wsData.push([]); 
        wsData.push([{v: "Report Generated By: GradeCentral", s: {font: {italic: true, sz: 9, color: {rgb: "FF7F8C8D"}}}}]);
