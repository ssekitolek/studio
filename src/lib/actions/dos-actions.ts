...
async function generatePdfDocument(
  ...
  currentY += 4; // Extra space before table
  ...
  if (headers.length > 0 && dataRows.length > 0) {
    autoTable(doc, {
      ...
      margin: { top: currentY },
      didDrawPage: (data) => { // For multi-page footer
        currentY = data.cursor?.y ?? margin; // Update currentY for footer on new pages
      }
    });
  ...
  // Loop through pages to add footer - jsPDF doesn't have a global footer for autoTable easily
    ...
    if (format === 'csv') {
      ...
      return { success: true, message: "CSV data prepared.", data: csvData };
        ...
        const wsData: (string | number | {v: string | number, s?: XLSX.CellStyle})[][] = [
            [{v: reportTitle, s: {font: {bold: true, sz: 16}}}],
            [{v: `Generated on: ${generationDate}`, s: {font: {italic: true}}}],
            [], 
        ];
        
        const headers = Object.keys(reportData[0]);
        wsData.push(headers.map(h => ({ v: h, s: { font: { bold: true }, fill: { fgColor: { rgb: "FFCCCCCC" } } } })));
        ...
        const pdfHeaders = Object.keys(reportData[0]);
        const pdfBody = reportData.map(row => Object.values(row).map(val => String(val ?? '')));
        const pdfMetadata = [{ label: "Generated on", value: generationDate }];
        const pdfBytes = await generatePdfDocument(reportTitle, pdfMetadata, pdfHeaders, pdfBody, reportFooter);
...
export async function downloadSingleMarkSubmission(submissionId: string, format: 'csv' | 'xlsx' | 'pdf'): Promise<{ success: boolean; message: string; data?: string | Uint8Array }> {
  ...
  try {
    ...
    const pdfMetadataItems = [
        { label: "Assessment", value: assessmentName },
        { label: "Submitted By", value: teacherNameForReport },
        { label: "Date Submitted", value: dateSubmitted },
        { label: "D.O.S. Status", value: submissionData.dosStatus || 'N/A'},
    ];
    if (submissionData.dosRejectReason) {
        pdfMetadataItems.push({ label: "Reject Reason", value: submissionData.dosRejectReason });
    }
    ...
    if (format === 'csv') {
      ...
      return { success: true, message: "CSV data prepared.", data: csvData };
        ...
        wsData.push([{v: `Marks for ${assessmentName}`, s:{font:{bold:true, sz:14}}}]);
        pdfMetadataItems.forEach(item => {
          wsData.push([{v: item.label, s:{font:{bold:true}}}, item.value]);
        });
        wsData.push([]); 

        const tableHeaders = Object.keys(studentMarksData[0]);
        wsData.push(tableHeaders.map(h => ({ v: h, s: { font: { bold: true }, fill: { fgColor: { rgb: "FFCCCCCC" } } } })));
        ...
        const colWidths = [ {wch: 20}, {wch: 30}, {wch: 10} ]; 
        ...
        const pdfTableHeaders = Object.keys(studentMarksData[0]);
        const pdfTableBody = studentMarksData.map(row => Object.values(row).map(val => String(val ?? '')));
        const pdfBytes = await generatePdfDocument(pdfReportTitle, pdfMetadataItems, pdfTableHeaders, pdfTableBody, reportFooter);
        return { success: true, message: "PDF document prepared.", data: pdfBytes };
    }
    return { success: false, message: "Invalid format selected." };
  } catch (error) {
