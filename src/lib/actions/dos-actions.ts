...
function generateTextTable(
    ...
    if (dataRows.length === 0 && headers.length === 0 && !title && (!metadataLines || metadataLines.length === 0)) {
        return "No data to display.";
    }

    const allStringRows = dataRows.map(row => row.map(cell => String(cell === null || cell === undefined ? 'N/A' : cell)));
    const stringHeaders = headers.map(h => String(h));

    const columnWidths: number[] = [];
    if (stringHeaders.length > 0) {
        for (let i = 0; i < stringHeaders.length; i++) {
            let maxWidth = stringHeaders[i].length;
            for (const row of allStringRows) {
    ...
    } else if (allStringRows.length > 0 && allStringRows[0].length > 0) {
        for (let i = 0; i < allStringRows[0].length; i++) {
            let maxWidth = 0;
            for (const row of allStringRows) {
                if (row[i] && row[i].length > maxWidth) {
                    maxWidth = row[i].length;
    ...
    if (metadataLines && metadataLines.length > 0) {
        metadataLines.forEach(line => output += line + "\n");
        output += "\n";
    }

    if (columnWidths.length === 0 && allStringRows.length === 0 && headers.length === 0) {
        if (output.trim() === "") return "No data to display.";
        output += "\nReport Generated By: GradeCentral\n";
        return output;
    }
    
    if (columnWidths.length === 0) { // No actual table data, but maybe title/metadata
        output += "\n(No tabular data for selected criteria)\n";
        output += "\nReport Generated By: GradeCentral\n";
        return output;
    }
    ...
    // Header row
    if (stringHeaders.length > 0) {
        output += "│" + stringHeaders.map((header, i) => ` ${header.padEnd(columnWidths[i])} `).join("│") + "│\n";
        // Separator
        output += "├" + columnWidths.map(w => "─".repeat(w + 2)).join("┼") + "┤\n";
    }
    ...
    if (allStringRows.length > 0) {
      allStringRows.forEach(row => {
        output += "│" + row.map((cell, i) => ` ${(String(cell)).padEnd(columnWidths[i])} `).join("│") + "│\n";
      });
    } else {
        // No data rows, but headers exist - show empty table line
        output += "│" + columnWidths.map(w => " ".repeat(w + 2)).join("│") + "│\n";
    }
  ...
  try {
    ...
    if (format === 'csv') {
      ...
      return { success: true, message: "CSV data prepared.", data: csvData };
        ...
        const wsData: (string | number | {v: string | number, s?: XLSX.CellStyle})[][] = [
            [{v: reportTitle, s: {font: {bold: true}}}], // Title row, bold
            [generationDate],                          // Generation date
            [],                                        // Blank row for spacing
        ];
        
        const headers = Object.keys(reportData[0]);
        wsData.push(headers.map(h => ({ v: h, s: { font: { bold: true }}}))); // Bold table headers

        reportData.forEach(row => {
            wsData.push(Object.values(row).map(val => String(val === undefined || val === null ? '' : val)));
        });
        wsData.push([]); // Blank row
        wsData.push([reportFooter]);


        const worksheet = XLSX.utils.aoa_to_sheet(wsData);
        
        // Attempt to set column widths (optional, might not be perfect)
        const colWidths = headers.map(header => ({wch: Math.max(header.length, 15)})); // Default width
        reportData.forEach(row => {
    ...
    } else if (format === 'pdf') { // Text format
      const headers = Object.keys(reportData[0]) as Array<keyof ReportRow>;
      const dataRows = reportData.map(row => headers.map(header => row[header]));
      const textContent = generateTextTable(headers, dataRows, reportTitle, [generationDate]);
      return { success: true, message: "Text report data prepared.", data: textContent };
    }
...
export async function updateSubmittedMarksByDOS(
  ...
  try {
    ...
    await updateDoc(submissionRef, {
      ...
      dosEdited: true, // Flag that D.O.S made an edit
      dosLastEditedAt: Timestamp.now(),
      // Optionally, reset D.O.S. status to 'Pending' if an edit implies re-review,
      // or keep it as is if edits are minor fixes. For now, keeping it as is.
      // dosStatus: 'Pending', 
      // dosRejectReason: null, // Clear reject reason if editing after rejection
    });
...
export async function downloadSingleMarkSubmission(submissionId: string, format: 'csv' | 'xlsx' | 'pdf'): Promise<{ success: boolean; message: string; data?: string | Uint8Array }> {
  ...
  try {
    ...
    if (submissionData.dosRejectReason) {
        metadataArray.push(["Reject Reason:", submissionData.dosRejectReason]);
    }
    ...
    if (format === 'csv') {
      ...
      return { success: true, message: "CSV data prepared.", data: csvData };
        ...
        metadataArray.forEach(metaRow => {
            wsData.push([{v: metaRow[0], s: {font: {bold: true}}}, metaRow[1]]);
        });
        wsData.push([]); // Empty row as separator

        const tableHeaders = Object.keys(studentMarksData[0]);
        wsData.push(tableHeaders.map(h => ({ v: h, s: { font: { bold: true }}}))); // Bold table headers
        ...
        const colWidths = [ {wch: 20}, {wch: 30} ]; // For metadata
        if (studentMarksData.length > 0) {
            const dataColWidths = Object.keys(studentMarksData[0]).map(header => ({wch: Math.max(header.length, 15)}));
             studentMarksData.forEach(row => {
                Object.values(row).forEach((val, i) => {
                    const len = String(val).length;
                    if (dataColWidths[i].wch < len) dataColWidths[i].wch = len;
                });
            });
            // This structure might need adjustment if metadata and data are in same columns
            // For simplicity, apply general widths or specific ones if layout is fixed
            worksheet['!cols'] = [ {wch: 20}, {wch: 30}, {wch: 10} ]; // Example fixed widths
        }


        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, assessmentNameSlug.substring(0,30) || "Submission");
        const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
        return { success: true, message: "XLSX data prepared.", data: new Uint8Array(excelBuffer) };

    } else if (format === 'pdf') { // Text Format
      const textReportTitle = `GradeCentral - Mark Submission Details`;
      const textMetadata = [
      ...
      if (submissionData.dosRejectReason) {
        textMetadata.push(`Reject Reason:   ${submissionData.dosRejectReason}`);
      }
      
      const tableHeaders = Object.keys(studentMarksData[0]);
      const tableDataRows = studentMarksData.map(row => Object.values(row));
      const textContent = generateTextTable(tableHeaders, tableDataRows, textReportTitle, textMetadata);
      return { success: true, message: "Text report data prepared.", data: textContent };
    }
    return { success: false, message: "Invalid format selected." };
  } catch (error) {
